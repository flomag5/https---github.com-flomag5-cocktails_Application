<template>
  <div>
    ...
    <table>
      ...
      <tbody>
        <tr v-for="(cocktail, index) in cocktails" :key="cocktail">
...
export default {
  ...
  methods: {
    goEdit(uid) {
      ...
      this.$router.push({ name: 'CocktailEdit', params: { id: uid } })
    },
    del(index) {
  ...
  computed: {
    comptage() {
      return (this.cocktails.length == 0) ? ' Aucun cocktail ' : ` Il y a ${this.cocktails.length} cocktails`
    },
    dateFormat() {
